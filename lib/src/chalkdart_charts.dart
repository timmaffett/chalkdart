...
class ChalkDartCharts {
  ...
  static List<String> dumpAllHWBTables() {
    chalk.level = 3; // force full color
    List<String> out = [];
    ...
    out.insert(
        0, chalk.black.onBrightWhite(center(chalk.bold('HWB Tables'), len)));
  ...
  static List<String> dumpHWBTable(List<String> out, String title, num hue) {
    ...
    for (num w = 0; w <= 1; w += .20) {
      ...
      for (num b = 0; b <= 1; b += .20) {
        if (w == 0) {
          if (b == 0) hb.write(chalk.black.onBrightWhite(r'W \ B'));
          hb.write(chalk.black.onBrightWhite(
              _padWithSpace((b * 100).toStringAsFixed(0) + '% ', 5)));
        }
        if (b == 0) {
          sb.write(chalk.black
              .onBrightWhite(_padWithSpace((w).toStringAsFixed(0) + '% ', 5)));
        }
    ...
    out.insert(firstLine, chalk.black.onBrightWhite(center(titleLine, len)));
    return out;
  }

  static List<String> dumpAllHSLTables() {
    Chalk.colorLevel = chalk.level = 3; // force full color
    List<String> out = [];
    ...
    final len = chalk.strip(out[0]).length;
    out.insert(
        0, chalk.black.onBrightWhite(center(chalk.bold('HSL Tables'), len)));
  ...
  static List<String> dumpHSLTable(List<String> out, String title, num hue,
    ...
    final titleLine2 = '${chalk.bold.black.onBrightWhite("($title)")}';
      ...
      for (var s = sStart;
        ...
        if (l == 1) {
          if (firstCol && !secondHalf) {
            hb.write(chalk.bold.black.onBrightWhite(r'L \ S'));
          }
          hb.write(
              chalk.bold.black.onBrightWhite(center(getColumnHeader(s), 5)));
        }
        if (firstCol && !secondHalf) {
          sb.write(chalk.bold.black.onBrightWhite(center(getRowHeader(l), 5)));
          firstCol = false;
        }
    ...
    out.insert(firstLine, chalk.black.onBrightWhite(center(titleLine, len)));
    out.insert(
        firstLine + 1, chalk.black.onBrightWhite(center(titleLine2, len)));
    return out;
  }
  ...
  static List<String> dumpAllHSVTables() {
    ...
    final len = chalk.strip(out[0]).length;
    out.insert(
        0, chalk.black.onBrightWhite(center(chalk.bold('HSV Tables'), len)));
  ...
  static List<String> dumpHSVTable(List<String> out, String title, num hue,
    ...
    final titleLine = '${chalk.bold("H=$hue°")}';
    final titleLine2 =
        title != '' ? '${chalk.bold.black.onBrightWhite("($title)")}' : '';
    ...
    for (num v = 1; v >= 0; v -= (1 / 8)) {
      ...
      for (var s = sStart;
        ...
        if (v == 1) {
          if (firstCol && !secondHalf) {
            hb.write(chalk.black.onBrightWhite(r'V \ S'));
          }
          hb.write(chalk.black.onBrightWhite(center(getColumnHeader(s), 5)));
        }
        if (firstCol && !secondHalf) {
          sb.write(chalk.bold.black.onBrightWhite(center(getRowHeader(v), 5)));
          firstCol = false;
        }
