...
void main(List<String> arguments) {
  ...
  final parser = ArgParser()
    ..addFlag("colorMethods", negatable: false, abbr: 'c')
...
*/
  ...
  colorKeywords.forEach((colorKeyword, hexColorValue) {
    ...
    out.add(
        "/// set foreground color to $colorSource color $colorKeyword <span style='background-color: $cssColorCode;border: black solid thick;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> (0x$colorHexStr)/rgb($red, $green, $blue)");
    out.add("Chalk get $entryPoint => _makeRGBChalk($red, $green, $blue);");
    out.add("");
    out.add(
        "/// set background color to $colorSource color $colorKeyword <span style='background-color: $cssColorCode;border: black solid thick;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> (0x$colorHexStr)/rgb($red, $green, $blue)");
    out.add(
        "Chalk get $onEntryPoint => _makeRGBChalk($red, $green, $blue, bg:true);");
    out.add("");
  });
...
List<String> X11ColorTable() {
  ...
  out.add(
      '<tr><th style="text-align: center;" colspan="${2 * numColumns}">Chalk X11/CSS Color Style Methods</th></tr>');
  ...
  for (int row = 0; row < lenColumn; row++) {
    ...
    for (int c = 0; c < numColumns; c++) {
      ...
      if ((entry < numX11s)) {
        ...
        if (hex != -2) {
          out.add(
              '<td title="$tooltip" style="outline:solid $colorCode 1px;background:$backgroundColor;border: solid $colorCode thick;color: $colorCode;">.$colorKeyword</td>');
          out.add(
              '</td><td style="outline:solid $colorCode 1px;border:solid $colorCode thick;background-color: $colorCode;color: $textColor;">.$onEntryPoint</td>');
        }
      } else {
        out.add('<td></td><td></td>');
      }
