...
- `cyan`
- `white`
- `brightBlack`, `blackBright`, `gray` or `grey`
- `brightRed` or `redBright`
- `brightGreen` or `greenBright`
- `brightYellow` or `yellowBright`
- `brightBlue` or `blueBright`
- `brightMagenta` or `magentaBright`
- `brightCyan` or `cyanBright`
- `brightWhite` or `whiteBright`

### Background colors
The can be access by prefixing color with 'on' or 'bg'.
For example 'onBlue' and 'bgBlue' are the same, it is a matter of
users preference on how they want there code to read.
